...
export function AppDownloadModal() {
  ...
  return (
    <div className="fixed inset-0 z-[95] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm">
      ...
      <section className="relative z-10 w-full max-w-sm overflow-hidden rounded-[28px] bg-white text-[#173251] shadow-[0_24px_80px_rgba(15,23,42,0.28)]" role="dialog" aria-modal="true" aria-labelledby="app-download-title">
        <button type="button" onClick={() => setOpen(false)} className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-slate-500 shadow-sm ring-1 ring-slate-200" aria-label={t("Close", "বন্ধ করুন")}>
          <X className="h-4 w-4" />
        </button>
        <div className="bg-gradient-to-br from-[#e6f4ff] via-[#f5fbff] to-[#dbe9ff] px-5 pb-5 pt-8 text-center">
          <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-[26px] bg-gradient-to-br from-[#61b8ff] to-[#3174db] text-white shadow-[0_14px_26px_rgba(49,116,219,0.25)]">
        ...
        <div className="p-4">
          ...
          <button type="button" onClick={install} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#8bd51f] to-[#5dbb19] px-4 py-3 text-base font-black text-white shadow-[0_10px_22px_rgba(93,187,25,0.22)]"><Download className="h-5 w-5" />{t("Install / download", "ডাউনলোড করুন")}</button>
        </div>
      </section>
    </div>
  );
