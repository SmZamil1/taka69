  ...
  return (
    <div className="-mx-3 -mt-3 min-h-[calc(100vh-5rem)] bg-[#eef5fb] px-3 pb-24 pt-3 text-[#173251]">
      <div className="mx-auto max-w-lg space-y-3">
        ...
        <div className="grid grid-cols-1 gap-2 min-[400px]:grid-cols-3">{[{ icon: <Zap className="h-4 w-4" />, label: t("Weekly bonus", "সাপ্তাহিক বোনাস"), value: `+${currentLevel.weeklyBonus} TK` }, { icon: <Star className="h-4 w-4" />, label: t("Rebate rate", "রিবেট রেট"), value: `${currentLevel.rebateRate}%` }, { icon: <Crown className="h-4 w-4" />, label: t("Withdraw limit", "উইথড্র লিমিট"), value: `${(currentLevel.withdrawLimit / 1000).toFixed(0)}K` }].map((b) => <div key={b.label} className="rounded-xl border border-[#dce8f2] bg-[#f8fbfe] p-3 text-[#173251] shadow-sm"><div className="mb-1 flex items-center gap-1.5 text-[#2675bd]">{b.icon}<span className="text-[10px] font-bold text-[#6d879f]">{b.label}</span></div><div className="text-sm font-black text-[#d4871b]">{b.value}</div></div>)}</div>
