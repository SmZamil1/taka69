...
export async function GET() {
  try {
    ...
    const full = await prisma.user.findUnique({
      where: { id: user.id },
      select: { vipLevel: true, vipExp: true, totalBet: true, lastDailyAt: true },
    });
    ...
    const expProgress = nextLevel
      ? Math.min(100, ((full.vipExp - currentLevel.minExp) / (nextLevel.minExp - currentLevel.minExp)) * 100)
      : 100;

    const canClaimDaily = !full.lastDailyAt ||
      Date.now() - full.lastDailyAt.getTime() >= 20 * 60 * 60 * 1000;

    return ok({
      vipLevel: full.vipLevel,
      vipExp: full.vipExp,
      expProgress: parseFloat(expProgress.toFixed(1)),
      currentLevel,
      nextLevel,
      canClaimDaily,
      levels: VIP_LEVELS,
