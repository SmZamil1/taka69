  ...
  try {
    ...
    const requests = await prisma.walletRequest.findMany({
      where: {
        userId: user.id,
        createdAt: { gte: since },
        ...(requestType === "DEPOSIT" || requestType === "WITHDRAW" ? { type: requestType } : {}),
      },
      orderBy: { createdAt: "desc" },
      take: 200,
      select: { id: true, type: true, method: true, channel: true, amount: true, grossAmount: true, feeAmount: true, netAmount: true, status: true, trxId: true, providerRef: true, bonusAmount: true, note: true, adminNote: true, rejectionReason: true, processedAt: true, createdAt: true },
    });
