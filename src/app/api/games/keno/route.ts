...
const PAYOUTS: Record<number, Record<number, number>> = {
  ...
  4:  { 2: 1.4, 3: 4.5, 4: 140 },
  5:  { 3: 2.2, 4: 13, 5: 470 },
  6:  { 3: 1.4, 4: 4.5, 5: 45, 6: 1400 },
  7:  { 4: 2.8, 5: 18, 6: 90, 7: 4500 },
...
function drawNumbers(count: number, exclude: Set<number>): number[] {
  const pool = Array.from({ length: 40 }, (_, i) => i + 1).filter(n => !exclude.has(n));
  const drawn: number[] = [];
...
export async function POST(req: Request) {
  try {
    ...
    // Draw 20 numbers (standard Keno)
    const drawn = drawNumbers(20, new Set());
