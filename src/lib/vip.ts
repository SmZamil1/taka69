...
/** VIP levels adapted from Upload_Code VipPlan + GOA level system */
...
/** Add VIP exp from betting. 1 TK bet = 1 exp */
export async function addVipExp(userId: string, betAmount: number) {
  ...
  if (levelUp) {
    ...
    const bonus = lvl.dailyBonus * 5; // 5-day bonus on level up
    await prisma.$transaction([
      prisma.user.update({ where: { id: userId }, data: { balance: { increment: bonus } } }),
      prisma.transaction.create({
        data: {
          ...
          note: `VIP Level Up to ${lvl.nameEn} — welcome bonus`,
